...
### ニュース監視
Google News RSSから特定キーワードに一致する記事を15分ごとにチェックし、指定チャンネルに自動投稿。
キーワードの追加・削除・通知先チャンネルはスラッシュコマンドで管理。
...
| コマンド | 説明 |
|---------|------|
| `/news channel <channel>` | ニュース通知先チャンネルを設定 |
| `/news add <keyword>` | 監視キーワードを追加 |
| `/news remove <keyword>` | 監視キーワードを削除 |
| `/news list` | 登録中のキーワード一覧を表示 |
